"""DAG sales_daily_mart: ежедневный запуск сохранённого SQL-запроса DataLens Platform.

Раз в сутки (cron "0 0 * * *", 00:00 UTC) выполняет сохранённый SQL-запрос DLP
ecgnn7kizpbax (построение витрины продаж; параметров у запуска нет) через
лежащий рядом SDK-модуль dlp_sdk (RPC runSqlQuery, окружение preprod, org
...
with DAG(
    dag_id="sales_daily_mart",
    description="Раз в сутки выполняет сохранённый SQL-запрос DLP (витрина продаж, preprod) через dlp_sdk",
